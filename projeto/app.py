...
with st.sidebar:
    ...
    botao_imdb = st.link_button(
        'IMDB', 'https://www.imdb.com'
    )  # Link para a página oficial do IMDB

    if botao_lista_de_filmes:
        st.write('Filmes')
        st.dataframe(
            df_filmes['Movie Name'],
            hide_index=True,
        )
...
with col_1:
    ...
    opcao_pesquisa = st.selectbox(
        label='Atributo para pesquisar filme',
        options=['Ano', 'Avaliação', 'Nome'],
        index=None,
    ...
    if opcao_pesquisa == 'Ano':
        ...
        ano = st.slider(
            label='Escolha o ano',
            min_value=df_filmes['Year of Release'].min(),
            max_value=df_filmes['Year of Release'].max(),
        )
    ...
    try:
        ...
        if botao_pesquisar:
            ...
            # --- Quando se tem a opção Avaliação
                ...
                indices_filmes = df_filmes[
                    (df_filmes['Movie Rating'] >= avaliacao[0])
